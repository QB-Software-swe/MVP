...
public class GetEmailMethodCallServiceTest {
    ...
    @Mock private GetEmailMethodResponseBuilderPort getEmailMethodResponseBuilderPort;
    ...
    @Mock private EmailPropertiesFilter emailPropertiesFilter;

    @Mock private ListMultimapPort<String, ResponseInvocationPort> previousResponses;

    @Mock private GetEmailMethodCallPort getEmailMethodCallPort;

    @Mock private GetEmailMethodResponsePort getEmailMethodResponsePort;

    @InjectMocks private GetEmailMethodCallService getEmailMethodCallService;
    ...
    public void testCallWithRetrive() throws Exception {
        ...
        when(emailRepository.retrive(any()))
        ...
        when(getEmailMethodResponseBuilderPort.reset())
                .thenReturn(getEmailMethodResponseBuilderPort);

        when(getEmailMethodResponseBuilderPort.list(any()))
                .thenReturn(getEmailMethodResponseBuilderPort);

        when(getEmailMethodResponseBuilderPort.notFound(any()))
                .thenReturn(getEmailMethodResponseBuilderPort);

        when(getEmailMethodResponseBuilderPort.state(any()))
                .thenReturn(getEmailMethodResponseBuilderPort);
        when(getEmailMethodResponseBuilderPort.build()).thenReturn(getEmailMethodResponsePort);

        GetEmailMethodResponsePort result =
    ...
    public void testCallWithRetriveAll() throws Exception {
        ...
        when(getReferenceIdsResolver.resolve(any(), any())).thenReturn(emailIds);
        when(emailRepository.retriveAll(any()))
                .thenReturn(new RetrivedEntity<>(emails, new String[] {}));
        when(getEmailMethodCallPort.accountId()).thenReturn(accountId);
        when(accountStateRepository.retrive(accountId)).thenReturn(accountState);
        when(emailPropertiesFilter.filter(any(), any(), any())).thenReturn(emails);
