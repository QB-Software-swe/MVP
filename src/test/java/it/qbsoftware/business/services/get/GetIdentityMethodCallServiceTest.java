...
public class GetIdentityMethodCallServiceTest {
    ...
    @Mock private GetIdentityMethodCallPort getIdentityMethodCallPort;

    @Mock private ReferenceIdsResolver getReferenceIdsResolver;

    @Mock private IdentityPropertiesFilter identityPropertiesFilter;

    @Mock private GetIdentityMethodResponsePort getIdentityMethodResponsePort;

    @Mock private GetIdentityMethodResponseBuilderPort getIdentityMethodResponseBuilderPort;

    @InjectMocks private GetIdentityMethodCallService getIdentityMethodCallService;
    ...
    public void testCallWithRetrive()
        ...
        IdentityPort[] identitys =
                new IdentityPort[] {new IdentityAdapter(null), new IdentityAdapter(null)};

        when(getReferenceIdsResolver.resolve(any(), any())).thenReturn(identityIds);
        when(identityRepository.retrive(any()))
                .thenReturn(new RetrivedEntity<>(identitys, new String[] {}));
        ...
        when(getIdentityMethodResponseBuilderPort.reset())
                .thenReturn(getIdentityMethodResponseBuilderPort);

        when(getIdentityMethodResponseBuilderPort.list(any()))
                .thenReturn(getIdentityMethodResponseBuilderPort);

        when(getIdentityMethodResponseBuilderPort.notFound(any()))
                .thenReturn(getIdentityMethodResponseBuilderPort);

        when(getIdentityMethodResponseBuilderPort.state(any()))
                .thenReturn(getIdentityMethodResponseBuilderPort);
        when(getIdentityMethodResponseBuilderPort.build())
                .thenReturn(getIdentityMethodResponsePort);

        GetIdentityMethodResponsePort result =
                getIdentityMethodCallService.call(getIdentityMethodCallPort, previousResponses);
    ...
    public void testCallWithRetriveAll()
        ...
        when(getReferenceIdsResolver.resolve(any(), any())).thenReturn(identityIds);
        when(identityRepository.retriveAll(any()))
                .thenReturn(new RetrivedEntity<>(identitys, new String[] {}));
        when(getIdentityMethodCallPort.accountId()).thenReturn(accountId);
        when(accountStateRepository.retrive(accountId)).thenReturn(accountState);
        when(identityPropertiesFilter.filter(any(), any())).thenReturn(identitys);
