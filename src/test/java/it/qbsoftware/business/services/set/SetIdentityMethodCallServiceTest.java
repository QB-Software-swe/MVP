...
public class SetIdentityMethodCallServiceTest {
    ...
    @Mock CreateIdentity createIdentity;
    @Mock SetIdentityMethodResponseBuilderPort setIdentityMethodResponseBuilderPort;

    @Mock AccountState accountState;

    @Mock CreatedResult<IdentityPort> createdResult;

    @Mock UpdatedResult<IdentityPort> updatedResult;

    @Mock DestroyedResult destroyedResult;

    @Mock private SetIdentityMethodCallPort setIdentityMethodCallPort;

    @Mock private SetIdentityMethodResponsePort setIdentityMethodResponsePort;

    @Mock private ListMultimapPort<String, ResponseInvocationPort> previousResponses;

    @InjectMocks private SetIdentityMethodCallService setIdentityMethodCallService;
    ...
    public void testCall() throws AccountNotFoundException, StateMismatchException {
        ...
        when(setIdentityMethodResponseBuilderPort.reset())
                .thenReturn(setIdentityMethodResponseBuilderPort);
        when(setIdentityMethodResponseBuilderPort.oldState(any()))
                .thenReturn(setIdentityMethodResponseBuilderPort);
        when(setIdentityMethodResponseBuilderPort.newState(any()))
                .thenReturn(setIdentityMethodResponseBuilderPort);
        when(setIdentityMethodResponseBuilderPort.created(any()))
                .thenReturn(setIdentityMethodResponseBuilderPort);
        when(setIdentityMethodResponseBuilderPort.notCreated(any()))
                .thenReturn(setIdentityMethodResponseBuilderPort);
        when(setIdentityMethodResponseBuilderPort.build())
                .thenReturn(setIdentityMethodResponsePort);

        SetIdentityMethodResponsePort result =
                setIdentityMethodCallService.call(setIdentityMethodCallPort, previousResponses);
